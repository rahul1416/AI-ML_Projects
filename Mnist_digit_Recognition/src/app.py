    ...
    if correct == 'Yes':
        ...
        with st.form("form"):
            ...
            if submitted:
                ...
                val = torch.flatten(tensor_image)
                new_row = {'label': option}
                for i in range(0, 784): 
                    new_row[f'pixel{i}'] = val[i].numpy()
                df = pd.DataFrame(new_row,index=[0])
                if os.path.exists("data/digit-recognizer/updated.csv"):
                    df.to_csv('data/digit-recognizer/updated.csv', mode='a', index=False, header=False)
                    df.to_csv('data/digit-recognizer/Feedback.csv', mode='a', index=False, header=False)
                else:
                    df.to_csv('data/digit-recognizer/updated.csv', mode='a', index=False)
                    df.to_csv('data/digit-recognizer/Feedback.csv', mode='a', index=False)
